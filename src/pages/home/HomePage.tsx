...
import onedayImg from '../../assets/oneday.png';
import t20Img from '../../assets/t20.png';
import superoverImg from '../../assets/superover.png';
import bowloutImg from '../../assets/bowlout.png';
import welcomeGift from '../../assets/welcome_gift.png';

import { EmptyState, AppHeader, BottomNav, CoinBadge } from '../../components/common';
import { CoinWalletPopup } from '../../components/common/CoinWalletPopup';
...
export default function HomePage() {
  ...
  const handleEventClick = (title: string, buttonText: string) => {
    if (buttonText === 'Play') {
      navigate(`/events/${encodeURIComponent(title)}/create-team`);
    } else {
      toast.info(`${title} is locked`, {
        description:
          "This prediction event is currently locked. Play 'Runs over 5 trading days' to start!",
      });
    }
  };
  ...
  return (
    <div className="h-full flex flex-col bg-[#F4F7FB] relative">
      ...
      <AppHeader
        ...
        rightContent={
          <>
            ...
            <CoinBadge balance={850} onClick={() => setShowCoinWallet(true)} variant="light" />
          </>
        }
      />
      ...
      <div className="flex-1 overflow-y-auto pb-24">
        ...
        <div
          className="mx-4 mt-3 rounded-[20px] relative overflow-hidden min-h-[120px] flex items-center shadow-premium-lg border border-white/10"
          style={{
            background:
              'linear-gradient(135deg, #6366F1 0%, #818CF8 25%, #A78BFA 45%, #E879A8 65%, #F59E0B 85%, #FB923C 100%)',
          }}
        >
          {/* Confetti Dots */}
          ...
          <div className="flex-1 pr-[140px] z-10 relative pl-5 py-4">
            <h2 className="text-[17px] font-extrabold text-white mb-0.5 tracking-tight drop-shadow-sm">
              Welcome Bonus
            </h2>
            <p className="text-[13px] text-white/90 mb-3 font-medium">Get 100 free coins!</p>
            <button
              onClick={() =>
                toast.success('100 Coins Claimed!', {
                  description: 'Your demo coin balance has been updated.',
                })
              }
          ...
          <div className="absolute right-[-8px] top-[-8px] bottom-[-8px] w-[160px] flex items-center justify-center z-10 pointer-events-none">
            <img
              src={welcomeGift}
              alt="Welcome Gift"
              className="h-full w-auto object-contain transform scale-[1.15]"
            />
          </div>
        </div>
          ...
          <div className="mx-4 mt-3 bg-white rounded-2xl p-4 shadow-premium-md border border-[#E5E7EB]/60 hover:shadow-premium-lg hover:-translate-y-0.5 transition-all duration-200">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0 relative pr-20">
                <p className="text-sm font-bold text-[#101828] mb-0.5">Your KR11 is active.</p>
                ...
                <button
                  onClick={() => {
                    ...
                    toast.success('KYC Verified', {
                      description: 'Your account is fully active for demo gameplay.',
                    });
                  }}
        ...
        <div className="px-4 mt-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <button
              onClick={() =>
                toast.info('All Events', {
                  description: 'You are viewing all available live stock fantasy events.',
                })
              }
...
function EventCard({ image, title, description, buttonText, badgeText, onPlay }: EventCardProps) {
  return (
